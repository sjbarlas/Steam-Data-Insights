...
--- b. Copying the data from the CSV file to the table in the postgres database
CREATE TABLE public."app_id_info" (appid integer, app_title text, app_type text, app_price money, 
								   release_date timestamp with time zone, app_rating smallint,
								   required_age smallint, is_multiplayer boolean)
...
--- b. Copying the data from the CSV file to the table in the postgres database
CREATE TABLE public."friends" (steamid_a bigint, steamid_b bigint, 
...
--- b. Copying the data from the CSV file to the table in the postgres database
CREATE TABLE public."games_1" (steamid bigint, appid integer, playtime_2weeks integer, 
							   playtime_forever integer, dateretrieved timestamp with time zone)
...
--- b. Copying the data from the CSV file to the table in the postgres database
CREATE TABLE public."games_2" (steamid bigint, appid integer, playtime_2weeks integer, 
							   playtime_forever integer, dateretrieved timestamp with time zone)
...
--- b. Copying the data from the CSV file to the table in the postgres database
CREATE TABLE public."player_summaries" (steamid bigint, personaname text, profileurl text, 
										avatar text, avatarmedium text, avatarfull text, personastate smallint,
										communityvisibilitystate smallint, profilestate smallint, 
										lastlogoff timestamp with time zone, commentpermission smallint, 
										realname text, primaryclanid bigint, timecreated timestamp with time zone, 
										gameid integer, gameserverip text, gameextrainfo text, 
										cityid integer, loccountrycode VARCHAR(4), 
										locstatecode text, loccityid integer, dateretrieved timestamp with time zone)
